...
class Product extends Model
{
    ...
    {
        return $this->morphToMany(Document::class, 'resource', 'document_resource')->withPivot('id', 'name', 'description', 'ext', 'size', 'mime','isactive','created_at');
    }


    public function getProductImageAttribute()
    {
        $document= $this->documents()->where('document_id', 1)->first();
        if ($document)
        {
