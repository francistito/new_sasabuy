...
//    Create mass assign using DB builder
    public function createMassAssignDbBuilder($table, array $input)
    {
        ...
        $entry_id=  DB::table($table)->insertGetId($input_common);
        return $entry_id;
    }
 ...
 * update image
 */
    public function updateImageUrl($modal,$document_id, $image_url_col ='image_url')
    {

        $image = code_value()->getImageTopPathAttribute($modal,$document_id);

        return DB::transaction(function() use($modal,$image,$image_url_col){
            $modal->update([
                $image_url_col => $image,
            ]);
            return $modal;
    ...
    public function saveDocumentsGeneral($resource_id, $document_id, $image_input_name,array $input)
    {
        $document_resource_repo = new DocumentResourceRepository();
        if((request()->file($image_input_name))){
            $document_resource_repo->saveDocument($resource_id,$document_id,$image_input_name, $input);
        }

    }

    /**
     * @param Model $page_container
     * @param array $input
     * Update sort
     */
    public function updateSortGeneral(Model $resource, array $input, $where_raw = null)
    {
        $where_raw = ($where_raw) ? $where_raw : 'id > 0';
        if($resource->sort != $input['old_sort']){
            $resource_with_new_sort = $this->query()->whereRaw($where_raw)->whereRaw("sort = ? and id <> ? ", [$input['sort'], $resource->id])->first();

            if($resource_with_new_sort){
                $resource_with_new_sort->update([
                    'sort' => $input['old_sort']
                ]);
            }
        }
     ...
     * Delete All documents general
     */
    public function deleteAllDocumentsGeneral(Model $resource)
    {
        $documents = $resource->documents;
        $doc_resource_repo = new DocumentResourceRepository();
        foreach ($documents as $document){
            $doc_resource_repo->deleteDocument($document->pivot->id);
        }
    }
