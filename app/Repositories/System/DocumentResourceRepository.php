...
class DocumentResourceRepository extends BaseRepository
{
    ...
    public function getTopDocFullPathUrl($doc_pivot_id)
    {
        $document_resource = $this->findDocumentResource($doc_pivot_id);
        $document = $this->getDocumentTypeFromPivotId($doc_pivot_id);
        $top_path = $document->documentGroup->top_path;
        $filename = $document_resource->id . '.' . $document_resource->ext;
        $full_url = top_base_doc_path() . $top_path  .  DIRECTORY_SEPARATOR . $document_resource->document_id . DIRECTORY_SEPARATOR .  $document_resource->resource_id  . DIRECTORY_SEPARATOR . $filename;
        return $full_url;
    }
...
//        $document_resource =  $resource->documents()->where('document_id', $document_id)->orderBy('id', 'desc')->first();
        ...
        /*Image url*/
        if($document_resource){
            $image_url = $this->getTopDocFullPathUrl($document_resource->id);
            DB::table('document_resource')->where('id', $document_resource->id)->update([
                'image_url' => $image_url
            ]);
        }
    ...
    public function deleteDocument($doc_pivot_id)
    {
        ...
        /*Non recurring- auto reset image url*/
        if($document->isrecurring == 0){
            $this->resetImageUrlForResource($document->id,$document_resource->resource_id);
        }
    }
     ...
     * @param $document_id
     * @param $doc_pivot_id
     * Reset image for non recurring doc
     */
    public function resetImageUrlForResource($document_id,$resource_id)
    {
        $resource = (new DocumentRepository())->getResourceInstance($document_id,$resource_id);
        if(isset($resource->image_url)){
            ...
            /*Resolve cl=ol image url*/
            $col_image_url = $resource->getImageColByDocument($document_id);
            if(isset($resource->$col_image_url)) {
                $resource->update([
                    $col_image_url => null
                ]);
            }
