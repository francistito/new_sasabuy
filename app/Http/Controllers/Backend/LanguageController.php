...
class LanguageController extends Controller
{
    ...
    public function store(Request $request)
    {
        if (Language::where('code', $request->code)->first()) {
            flash( ('This code is already used for another language'))->error();
            return back();
        }
    ...
    public function update(Request $request)
    {
        ...
        Cache::forget('languages');
        flash( ('Language has been updated successfully'))->success();
        return back();
    }
    ...
    public function updateStatus(Request $request)
    {
        ...
        if (env('DEFAULT_LANGUAGE') == $language->code && $request->is_active == 0) {
            return [
                'status'    => false,
                'message'    =>  ('Default language can not be disabled'),
            ];
        } elseif ($activatedLanguages <= 1 && $request->is_active == 0) {
            return [
                'status'    => false,
                'message'    =>  ('Minimum 1 language need to be enabled'),
            ];
        }
    ...
    public function key_value_store(Request $request)
    {
        ...
        flash( ('Localizations have been updated'))->success();
        return back();
    }
}
