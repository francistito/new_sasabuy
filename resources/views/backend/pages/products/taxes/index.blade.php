...
@section('title')
    {{  ('Taxes') }} {{ getSetting('title_separator') }} {{ getSetting('system_title') }}
@endsection
            ...
            <div class="row mb-4 g-4">
                        ...
                        <div class="col-12">
                            <div class="card mb-4" id="section-1">
                                                        ...
                                                        <div class="dropdown-menu dropdown-menu-end shadow">
                                                            @can('edit_taxes')
                                                                <a class="dropdown-item"
                                                                    href="{{ route('admin.taxes.edit', ['id' => $tax->id, 'lang_key' => env('DEFAULT_LANGUAGE')]) }}& ">
                                ...
                                <div class="d-flex align-items-center justify-content-between px-4 pb-4">
                                    <span>{{  ('Showing') }}
                                        {{ $taxes->firstItem() }}-{{ $taxes->lastItem() }} {{  ('of') }}
                                        {{ $taxes->total() }} {{  ('results') }}</span>
                                    <nav>
                    ...
                    @can('add_taxes')
                        <form action="{{ route('admin.taxes.store') }}" method="POST" enctype="multipart/form-data"
                            ...
                            <div class="card mb-4" id="section-2">
                                <div class="card-body">
                                    <h5 class="mb-4">{{  ('Add New Taxes') }}</h5>

                                    <div class="mb-4">
                                        <label for="name" class="form-label">{{  ('Tax Name') }}</label>
                                        <input type="text" name="name" id="name"
                                            placeholder="{{  ('Type tax name') }}" class="form-control" required>
                                    </div>
                                </div>
                            </div>
                            <!--tax info end-->
                ...
                <div class="col-xl-3 order-1 order-md-1 order-lg-1 order-xl-2">
                    <div class="card tt-sticky-sidebar">
                        <div class="card-body">
                            <h5 class="mb-4">{{  ('Tax Information') }}</h5>
                            <div class="tt-vertical-step">
                                <ul class="list-unstyled">
                                    <li>
                                        <a href="#section-1" class="active">{{  ('All Taxes') }}</a>
                                    </li>
                                    @can('add_taxes')
                                        <li>
                                            <a href="#section-2">{{  ('Add New Tax') }}</a>
                                        </li>
                                    @endcan
                                </ul>
                            </div>
